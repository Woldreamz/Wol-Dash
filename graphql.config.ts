...
const config: IGraphQLConfig = {
  ...
  extensions: {
    ...
    // https://the-guild.cdev/graphql/codegen`
    codegen: {
      ...
      generates: {
        ...
        "src/graphql/schema.types.ts": {
          ...
          plugins: ["typescript"],
          // set the config pf the typescript plugin
          // this defines how the generated types will look like
          config: {
            skipTypename: true,
            // skipTypename is used to remove __typename from the generated types
            enumsAsTypes: true,
            // enumsAsTypes is used to generate enums as types instead of enums.
        ...
        "src/graphql/types.ts": {
          ...
          // https://the-guild.dev/graphql/codegen/plugins/presets/import-types-presets 
          preset: "import-types",
          // documents is used to define the path of the files that contain GraphQL operationsn
          documents: ["src/**/*.{ts,tsx}"],
          // pluginsis used to define the plugins that will be used to generate typescript types from GraphQL operations.
          plugins: ["typescript-operations"],
          config: {
            skipTypename: true,
            enumsAsTypes: true,
            // determine whether the generated types should be resolved aahead of time or not.
            // When preResolveTypes is set to false, the code generator will not try to resolve the types ahead of time. 
            // Instad, it will generate more generic types, and the actual types will be resolved at runtime.
