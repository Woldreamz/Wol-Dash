...
export const authProvider: AuthProvider = {
  login: async ({ email }) => {
    try {
        // Call the login mututation
        // dataProvider.custom is used to make a custom request to the GraphQL API
        // this will call dataProvider which will go throguh the fetchWrapper function
      const { data } = await dataProvider.custom({
        ...
        meta: {
          variables: { email },
          // pass the email to see if the user exisits and if so, return the accessToken
          rawQuery: `
      ...
      // save the accessToken localStorage
